...
function Extract-HeyBenchmarkData {
    param (
        [string]$FilePath
    )
...
function Format-ComparisonTable {
    ...
    foreach ($metric in $metrics) {
        ...
        $percentDiff = if ($metric.Data2 -ne 0) { ($diff / $metric.Data2) * 100 } else { 0 }
        ...
        # Formatar valores
        $data1ValueStr = "{0:$($metric.Format)}" -f $metric.Data1
        $data2ValueStr = "{0:$($metric.Format)}" -f $metric.Data2
...
$apigwData = Extract-HeyBenchmarkData -FilePath $apigwResults
$ingressData = Extract-HeyBenchmarkData -FilePath $ingressResults
$echoApigwData = Extract-HeyBenchmarkData -FilePath $echoApigwResults
$echoIngressData = Extract-HeyBenchmarkData -FilePath $echoIngressResults
...
$latencyDiffPercent = if ($ingressData.Average -ne 0) { 
    (($apigwData.Average - $ingressData.Average) / $ingressData.Average) * 100 
} else { 
    0 
}

$throughputDiffPercent = if ($ingressData.RequestsPerSecond -ne 0) { 
...
if (([Math]::Abs($throughputDiffPercent)) -lt 5) {
    ...
    Write-Host "- Throughput: O API Gateway processa $([Math]::Abs($throughputDiffPercent).ToString("F1"))% mais requisições por segundo que o Serviço Direto" -ForegroundColor Green
} else {
    Write-Host "- Throughput: O Serviço Direto processa $([Math]::Abs($throughputDiffPercent).ToString("F1"))% mais requisições por segundo que o API Gateway" -ForegroundColor Green
}

# Calcular overhead do API Gateway
$echoLatencyDiffPercent = if ($echoIngressData.Average -ne 0) { 
    (($echoApigwData.Average - $echoIngressData.Average) / $echoIngressData.Average) * 100 
} else { 
    0 
}

Write-Host "- Overhead do API Gateway: No teste Echo (baixo processamento), o API Gateway adiciona aproximadamente $([Math]::Abs($echoLatencyDiffPercent).ToString("F1"))% de latência adicional" -ForegroundColor Cyan
