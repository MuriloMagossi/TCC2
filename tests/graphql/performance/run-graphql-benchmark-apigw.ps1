...
# Função para verificar se o serviço está acessível
function Test-ServiceAvailability {
    ...
    try {
        ...
        return $content -match '"hello":'
    }
    catch {
        Write-Host "Erro ao testar disponibilidade do serviço: $_" -ForegroundColor Red
        return $false
    }
}

# Função para iniciar port-forward com retry
function Start-PortForwardWithRetry {
    param (
        [string]$Service,
        [int]$LocalPort,
        [string]$Namespace,
    ...
    while ($retryCount -lt $MaxRetries) {
        try {
            Write-Host "Tentativa $($retryCount + 1) de iniciar port-forward..." -ForegroundColor Yellow
            $portForwardProcess = Start-Process -FilePath "kubectl" -ArgumentList "port-forward svc/$Service $($LocalPort):80 -n $Namespace" -PassThru -NoNewWindow
            
            # Aguardar conexão ser estabelecida
            Start-Sleep -Seconds 3
            
            # Testar se o serviço está acessível
            if (Test-ServiceAvailability -Url "http://localhost:$LocalPort/graphql") {
                Write-Host "Port-forward estabelecido com sucesso!" -ForegroundColor Green
                return $portForwardProcess
            }
            
            # Se chegou aqui, o serviço não está acessível
            ...
            if ($retryCount -lt $MaxRetries) {
                Write-Host "Aguardando $RetryDelay segundos antes da próxima tentativa..." -ForegroundColor Yellow
                Start-Sleep -Seconds $RetryDelay
            }
        }
        catch {
            Write-Host "Erro ao iniciar port-forward: $_" -ForegroundColor Red
            $retryCount++
            if ($retryCount -lt $MaxRetries) {
                Write-Host "Aguardando $RetryDelay segundos antes da próxima tentativa..." -ForegroundColor Yellow
                Start-Sleep -Seconds $RetryDelay
    ...
    throw "Não foi possível estabelecer o port-forward após $MaxRetries tentativas"
}

# Iniciar port-forward para API Gateway GraphQL
Write-Host "==> Iniciando port-forward para GraphQL API Gateway..." -ForegroundColor Cyan
$apigwPort = 9601
try {
    $portForwardProcess = Start-PortForwardWithRetry -Service "nginx-apigw-graphql" -LocalPort $apigwPort -Namespace "graphql" -MaxRetries $maxRetries -RetryDelay $retryDelay
}
catch {
...
Write-Host "==> Executando benchmark GraphQL para API Gateway..." -ForegroundColor Cyan

# Configurar URL e payload da consulta
$url = "http://localhost:$apigwPort/graphql"
...
function Invoke-ConcurrentGraphQLQueries {
    ...
    for ($i = 0; $i -lt $ConcurrentClients; $i++) {
        ...
        $job = Start-Job -ScriptBlock {
            param($url, $query, $queryCount, $maxDuration, $maxRetries)
            ...
            return $results
        } -ArgumentList $Url, $Query, $queryCount, $DurationSec, $MaxRetries
...
$resultStr | Out-File -FilePath "$resultsDir/hey-graphql-apigw-result.txt" -Encoding utf8
$resultStr | ForEach-Object { Write-Host $_ }

# Testar o endpoint echo para comparação
Write-Host "==> Executando benchmark para o endpoint GraphQL Echo API Gateway..." -ForegroundColor Cyan
$echoUrl = "http://localhost:$apigwPort/graphql/echo"
...
$echoResultStr | Out-File -FilePath "$resultsDir/hey-graphql-echo-apigw-result.txt" -Encoding utf8

# Melhorar o cleanup no final do script
try {
    ...
    Write-Host "Erro durante o cleanup: $_" -ForegroundColor Red
}
finally {
    Write-Host "==> Benchmark GraphQL API Gateway concluído!" -ForegroundColor Green
